...
declare global {
  var __KITE_TEST_DIR__: string
}

globalThis.__KITE_TEST_DIR__ = ''

// Create a unique temporary directory for each test
function createTestDir(): string {
  const dir = path.join(
    os.tmpdir(),
    'kite-test-' + Date.now() + '-' + Math.random().toString(36).slice(2)
  )
  globalThis.__KITE_TEST_DIR__ = dir
  return dir
}
...
vi.mock('os', async (importOriginal) => {
  ...
  return {
    ...actual,
    homedir: () => globalThis.__KITE_TEST_DIR__ || '/tmp/kite-test-fallback'
  }
})
...
vi.mock('electron', () => {
  return {
    app: {
      getPath: (name: string) => {
        const dir = globalThis.__KITE_TEST_DIR__ || '/tmp/kite-test-fallback'
        if (name === 'home') return dir
        if (name === 'userData') return path.join(dir, '.kite')
        return dir
      },
...
afterEach(() => {
  const testDir = globalThis.__KITE_TEST_DIR__
...
export function getTestDir(): string {
  return globalThis.__KITE_TEST_DIR__
}

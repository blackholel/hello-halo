...
Halo 是 Claude Code 的桌面 GUI 客户端，基于 Electron + React + TypeScript 构建。它封装了 Claude Agent SDK，提供可视化界面，无需使用终端即可进行 AI 辅助编程。
...
┌─────────────────────────────────────────────────────────────┐
│                    渲染进程 (React)                          │
│  Zustand Stores ◄── api layer ◄── window.halo (preload)     │
└─────────────────────────────────────────────────────────────┘
                          │ IPC
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
- `src/renderer/` - 渲染进程 (React)
  ...
  - `components/` - React 组件
  - `pages/` - 页面组件
- `src/preload/` - 预加载脚本，暴露 `window.halo` API
...
| ConfigService | `src/main/services/config.service.ts` | 配置管理（~/.halo/config.json） |
...
预加载脚本（`src/preload/index.ts`）定义了 `HaloAPI` 接口，向渲染进程暴露所有主进程功能。

### Agent SDK 集成

Halo 使用 `@anthropic-ai/claude-agent-sdk` npm 包，该 SDK 内置了 Claude Code 核心逻辑。子进程通过 Electron 自身（设置 `ELECTRON_RUN_AS_NODE=1`）运行，**不依赖系统安装的 `claude` CLI**。用户只需提供 API Key 即可使用。
...
Halo **默认不读取系统 Claude Code 配置**（`~/.claude/`），所有配置来自 Halo 自己的配置体系：

| 配置类型 | 来源 | 说明 |
|----------|------|------|
| Plugins | 可配置路径 | 默认加载 `~/.halo/plugins/`、`~/.halo/skills/` 和 `{workDir}/.claude/` |
| MCP Servers | Halo 全局配置 + Space 配置 | 不读取系统 MCP 配置 |
| Agents | `config.claudeCode.agents` + `spaceConfig.agents` | Halo 独立配置 |
| Hooks | `config.claudeCode.hooks` + `spaceConfig.hooks` | Halo 独立配置 |
| Memory | `~/.halo/` + `{workDir}/CLAUDE.md` | Halo 独立管理 |
...
**Space 配置** (`{workDir}/.halo/space-config.json`)：
```json
{
  "claudeCode": {
    "plugins": {
...
3. `~/.halo/plugins/` - 默认 app 级别路径
4. `~/.halo/skills/` - 默认 app 级别路径（与 plugins 同时加载）
5. Space `paths` - Space 自定义路径
6. `{workDir}/.claude/` - 默认 space 级别路径（Claude Code 兼容）

### 远程访问

HTTP 服务器 + WebSocket 支持从任意浏览器/设备控制 Halo。渲染进程的 api 层自动检测传输模式（桌面端使用 IPC，远程使用 HTTP）。

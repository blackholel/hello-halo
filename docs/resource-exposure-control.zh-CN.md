...
1. `public`：可展示、可直接调用（前提是资源来源在执行层允许范围内）
2. `internal-only`：不在外层展示；不可被用户直接 `/` 或 `@` 调用；但允许作为 command 依赖被内部调用

> 说明（2026-02-28）：执行层来源策略与展示策略分离。  
> 当前执行层会从 `~/.kite`（app/global/installed/plugin）和当前空间 `workDir/.claude`（space）共同解析资源；  
> `resource-exposure` 仍只负责“是否可直接调用/展示”的权限语义。

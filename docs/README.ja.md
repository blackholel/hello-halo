...
> **Kite** は Claude Code CLI をビジュアル AI コンパニオンに変えます。
...
### 本物の Agent ループ
チャットだけではありません。Kite は **実際に行動できます** — コードを書き、ファイルを作成し、コマンドを実行し、タスクが完了するまで反復します。
...
### リモートアクセス
スマートフォンや任意のブラウザからデスクトップの Kite を制御。どこからでも作業 — 病院のベッドからでも（実話）。
...
| プラットフォーム | ダウンロード | 要件 |
|----------|----------|--------------|
| **macOS** (Apple Silicon) | [.dmg をダウンロード](https://github.com/blackholel/buddykite/releases/latest) | macOS 11+ |
| **macOS** (Intel) | 近日公開 | macOS 11+ |
| **Windows** | [.exe をダウンロード](https://github.com/blackholel/buddykite/releases/latest) | Windows 10+ |
| **Linux** | [.AppImage をダウンロード](https://github.com/blackholel/buddykite/releases/latest) | Ubuntu 20.04+ |
...
1. **Kite を起動** して API キーを入力（Anthropic 推奨）
...
Kite は `~/.kite/` のみをサポートします。
旧 `~/.halo/` は読み込まれず、自動移行も行いません。

### アップグレードガイド（手動移行）

旧データが必要な場合は、Kite を終了した状態で `~/.halo/` から `~/.kite/` へ必要なファイル/ディレクトリを手動でコピーしてください:

- `config.json`
- `spaces/`
- `temp/`
- `skills/`, `commands/`, `agents/`, `plugins/`（使用している場合）
...
Kite は開発者だけのものではありません。私たちが見てきたもの：
...
障壁はもはや AI の能力ではありません。**アクセシビリティです。** Kite はその障壁を取り除きます。
...
- [GitHub Discussions](https://github.com/blackholel/buddykite/discussions) — 質問＆アイデア
- [Issues](https://github.com/blackholel/buddykite/issues) — バグレポート＆機能リクエスト
...
Kite が素晴らしいものを作る手助けになったら、ぜひ聞かせてください。

**このリポジトリに Star を** して、他の人が Kite を発見する手助けをしてください。

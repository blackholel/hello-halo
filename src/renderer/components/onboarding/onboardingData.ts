 ...
 * - AI response text
 * - HTML artifact content (a beautiful PPT introducing Kite)
 *
...
export const ONBOARDING_ARTIFACT_NAME = 'kite-intro.html'
...
export function getOnboardingPrompt(translator?: TFunction): string {
  ...
  return t(
    'Help me create an HTML slide deck introducing Kite (1920x1080) with a clean, modern style. Include: 1) What is Kite 2) Core capabilities 3) Use cases 4) Getting started'
  )
}
...
export function getOnboardingAiResponse(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t(
    "Sure, I'll create a polished HTML presentation that introduces Kite's core features and how to use it. The slides use a modern, minimal design, support left/right arrow navigation, and fit 1920x1080."
  )
}

export function getOnboardingConversationTitle(translator?: TFunction): string {
  const t = getTranslator(translator)
  return t('Welcome to Kite')
}

export function getOnboardingHtmlArtifact(translator?: TFunction, lang: string = getCurrentLanguage()): string {
  const t = getTranslator(translator)

  const docTitle = t('Kite - AI that gets things done')
  const tagline = t('AI that gets things done')
  const whatIsKite = t('What is Kite?')
  const moreThanChatTitle = t('More than chat')
  const moreThanChatDesc = t('Kite understands your goals, plans tasks, and can execute autonomously—not just answer questions.')
  ...
  const naturalLangDesc = t('Describe what you want in plain language; Kite will understand and act.')
  ...
  const writeCodeDesc = t('Describe your needs and Kite will generate complete code across languages and frameworks.')
  ...
  const webDesc = t('Need fresh information? Kite can search online and summarize what matters.')
  const docsTitle = t('Write documents')
  const docsDesc = t('Reports, proposals, emails—tell Kite what you need and it will draft it.')
  ...
  const startText = t('Start using Kite')

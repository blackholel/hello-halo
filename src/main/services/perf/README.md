...
await window.kite.perfStart()   // Start monitoring
await window.kite.perfStop()    // Stop monitoring
```
...
```javascript
await window.kite.perfGetState()     // Get current state
await window.kite.perfClearHistory() // Clear history
await window.kite.perfExport()       // Export as JSON
```

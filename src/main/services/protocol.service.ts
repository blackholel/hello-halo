 ...
 *
 * Provides kite-file:// protocol to bypass cross-origin restrictions when loading
 * local files from localhost (dev mode) or app:// (production mode).
 *
 * Usage:
 * - Images: <img src="kite-file:///path/to/image.png">
 * - PDF: BrowserView.loadURL("kite-file:///path/to/doc.pdf")
...
export function registerProtocols(): void {
  // kite-file:// - Proxy to file:// for local resources
  // Chromium blocks file:// from localhost/app origins, this bypasses that
  protocol.handle('kite-file', (request) => {
    const filePath = decodeURIComponent(request.url.replace('kite-file://', ''))
    return net.fetch(`file://${filePath}`)
  })

  console.log('[Protocol] Registered kite-file:// protocol')
}
